...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let stream = Box::pin(futures::stream::unfold(limit * 85, |state| async move {
        ...
        let result = req_with_backoff(&client, &get_url(base, state, limit), 5)
            .await
            .unwrap()
            .text()
            .await
    ...
    fs::create_dir_all(format!("datasets/{base}"))?;

    println!("Loading dataset {base}...");
    ...
    while let Some(feed) = stream.next().await {
        ...
        for entry in feed.entry {
            ...
            if let Err(e) = download(&client, base, id).await {
